...
InaturalistAPI.prepareApp = a => {
  ...
  a.use( InaturalistAPI.lookupObservedByUserMiddleware );
...
InaturalistAPI.server = ( ) => {
  ...
  if ( config.imageProcesing && config.imageProcesing.tensorappURL
    ...
    app.post( "/v1/rapidapi/score_image", upload.single( "image" ), ( req, res ) => {
      InaturalistAPI.preloadAndProcess( RapidAPIController.scoreImage, req, res, {
        async: true, preload: { localeDefaults: true }
      } );
    } );
  ...
  // Trips
  // FIXME add a GET endpoint for all trips
  // FIXME add GET for single trip
  dfault( "get", "/v1/trips", TripsController.index);
  dfault( "post", "/v1/trips", TripsController.create);
  dfault( "put", "/v1/trips/:id", TripsController.update);
  dfault( "delete", "/v1/trips/:id", TripsController.delete );
...
InaturalistAPI.lookupObservedByUserMiddleware = ( req, res, next ) => {
  InaturalistAPI.lookupInstanceMiddleware( req, res, next,
    "observed_by_user_id", User.findByLoginOrID, "observedByUser" );
};
...
InaturalistAPI.lookupInstanceMiddleware = ( req, res, callback, paramKey, method, objectKey ) => {
  if ( req.method === "OPTIONS" || req.inat.isV2 ) { return void callback( ); }
  req.inat = req.inat || { };
